	...
	<div class="table_container">
		<div class="btn_group">
			...
			<el-select v-model="condition" placeholder="请选择筛选条件">
				<el-option v-for="(item,index) in columns" :key="`option_${index}`" :label="item.label" :value="item.prop"></el-option>
			</el-select>
		...
		<div class="table_content">
			<el-table id="applyTable" :lazy="isLazy" :load="load" :data="data" :height='height' @header-dragend='surverWidth'
			 :empty-text="empty_text" style="width: 100%" row-key="id" :tree-props="treeProps" border ref="table">
				<el-table-column v-for="(item,index) in columnList" :key="Math.random()" :prop="item.prop" :label="columnNames[index]"
         :width="item.width || ''" :fixed="item.isFixed || false" :sortable="item.isSort || false" v-if="checkItems[index].isChecked">
				</el-table-column>
			</el-table>
		</div>
		<el-dialog title="筛选列" :visible.sync="dialogVisible" width="30%">
			<el-checkbox v-for="(item,index) in checkList" :key="index" v-model="item.isChecked">{{ item.label }}</el-checkbox>
